...
const getNews = async () => {
  try {
    let header = new Headers({
      'x-api-key': '0OCl5LSY7HDJF59_Iwm0H8LKYtUvqOh0CrwwSE9zbYk',
    });
    ...
    if (response.status == 200) {
      if (data.total_hits == 0) {
        throw new Error('Qidirgan Malumot mavjud emas');
      }
    ...
    console.log('ushlangan error', error.message);
    errorRender(error.message);
  }
};
// Api ni chaqirish funksiyasi
